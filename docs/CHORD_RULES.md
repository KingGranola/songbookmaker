...
- **v0.7.0** - 実装されたプリセットコードに以下を追加：
  - **裏コード（トライトーン・サブスティテューション）**: V7の代理コード
  - **II-V-I パッケージ**: ジャズの基本進行を一括生成
  - **サスペンドコード**: Sus4/Sus2による緊張と解決
- **v0.7.1** - UI改善：
  - **カテゴリ別ボックスレイアウト**: 視認性向上のため各カテゴリをボックス化
  - **グリッドレイアウト**: コードチップを整列された格子状に配置
  - **ホバー効果**: マウスオーバー時のアニメーション追加
