...
- **`modules/bootstrap.js`**: 全モジュールの初期化・統合
- **`modules/state.js`**: 基本的な状態管理・永続化
- **`modules/state-manager.js`**: リアクティブ状態管理システム
- **`modules/ui.js`**: UIイベント・DOM操作
- **`modules/placement.js`**: コード配置・ドラッグ&ドロップ
- **`modules/presets.js`**: ダイアトニックコード生成

#### 拡張機能
- **`modules/keyboard-shortcuts.js`**: キーボードショートカット管理
- **`modules/touch-gestures.js`**: タッチジェスチャー処理
- **`modules/performance.js`**: パフォーマンス最適化ユーティリティ
...
#### スタイルシート
- **`styles/base.css`**: CSS変数・基本スタイル
- **`styles/mobile.css`**: モバイル専用最適化
- **`styles/accessibility.css`**: アクセシビリティ対応
- **`styles/print.css`**: PDF出力専用スタイル
